...
2. Mark basic lambda calculus theorems. The bare minimum is the identity, constant, and composition
   ...
   ```
   The constant theorem is not absolutely necessary as, for example, `IsLinearMap ℝ (fun x ↦ y)`
   does not hold, but we almost certainly want to mark it if it is available.
