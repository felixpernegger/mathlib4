...
def jacobiTheta₂Term (n : ℤ) (z τ : ℂ) : ℂ := cexp (2 * π * I * n * z + π * I * n ^ 2 * τ)

@[deprecated (since := "2026-07-18")]
alias jacobiTheta₂_term := jacobiTheta₂Term

/-- Summand in the series for the Fréchet derivative of the Jacobi theta function. -/
def jacobiTheta₂TermFderiv (n : ℤ) (z τ : ℂ) : ℂ × ℂ →L[ℂ] ℂ :=
  cexp (2 * π * I * n * z + π * I * n ^ 2 * τ) •
    ((2 * π * I * n) • (ContinuousLinearMap.fst ℂ ℂ ℂ) +
      (π * I * n ^ 2) • (ContinuousLinearMap.snd ℂ ℂ ℂ))

@[deprecated (since := "2026-07-18")]
alias jacobiTheta₂_term_fderiv := jacobiTheta₂TermFderiv

lemma hasFDerivAt_jacobiTheta₂_term (n : ℤ) (z τ : ℂ) :
    HasFDerivAt (fun p : ℂ × ℂ ↦ jacobiTheta₂Term n p.1 p.2)
    (jacobiTheta₂TermFderiv n z τ) (z, τ) := by
...
def jacobiTheta₂'Term (n : ℤ) (z τ : ℂ) := 2 * π * I * n * jacobiTheta₂Term n z τ
...
We show that the sums of the three functions `jacobiTheta₂Term`, `jacobiTheta₂'Term` and
`jacobiTheta₂TermFderiv` are locally uniformly convergent in the domain `0 < im τ`, and diverge
everywhere else.
-/

lemma norm_jacobiTheta₂_term (n : ℤ) (z τ : ℂ) :
    ‖jacobiTheta₂Term n z τ‖ = rexp (-π * n ^ 2 * τ.im - 2 * π * n * z.im) := by
  rw [jacobiTheta₂Term, Complex.norm_exp, (by push_cast; ring :
    (2 * π : ℂ) * I * n * z + π * I * n ^ 2 * τ = (π * (2 * n) :) * z * I + (π * n ^ 2 :) * τ * I),
    add_re, mul_I_re, im_ofReal_mul, mul_I_re, im_ofReal_mul]
  ring_nf

/-- A uniform upper bound for `jacobiTheta₂Term` on compact subsets. -/
lemma norm_jacobiTheta₂_term_le {S T : ℝ} (hT : 0 < T) {z τ : ℂ}
    (hz : |im z| ≤ S) (hτ : T ≤ im τ) (n : ℤ) :
    ‖jacobiTheta₂Term n z τ‖ ≤ rexp (-π * (T * n ^ 2 - 2 * S * |n|)) := by
...
lemma norm_jacobiTheta₂'_term_le {S T : ℝ} (hT : 0 < T) {z τ : ℂ}
    (hz : |im z| ≤ S) (hτ : T ≤ im τ) (n : ℤ) :
    ‖jacobiTheta₂'Term n z τ‖ ≤ 2 * π * |n| * rexp (-π * (T * n ^ 2 - 2 * S * |n|)) := by
  rw [jacobiTheta₂'Term, norm_mul]
...
lemma summable_jacobiTheta₂_term_iff (z τ : ℂ) : Summable (jacobiTheta₂Term · z τ) ↔ 0 < im τ := by
  ...
  · by_contra! hτ
    ...
    · -- easy case `im τ < 0`
      suffices Tendsto (fun n : ℕ ↦ ‖jacobiTheta₂Term ↑n z τ‖) atTop atTop by
...
lemma norm_jacobiTheta₂_term_fderiv_le (n : ℤ) (z τ : ℂ) :
    ‖jacobiTheta₂TermFderiv n z τ‖ ≤ 3 * π * |n| ^ 2 * ‖jacobiTheta₂Term n z τ‖ := by
  -- this is slow to elaborate so do it once and reuse:
  have hns (a : ℂ) (f : (ℂ × ℂ) →L[ℂ] ℂ) : ‖a • f‖ = ‖a‖ * ‖f‖ := norm_smul a f
  rw [jacobiTheta₂TermFderiv, jacobiTheta₂Term, hns,
...
lemma norm_jacobiTheta₂_term_fderiv_ge (n : ℤ) (z τ : ℂ) :
    π * |n| ^ 2 * ‖jacobiTheta₂Term n z τ‖ ≤ ‖jacobiTheta₂TermFderiv n z τ‖ := by
  have : ‖(jacobiTheta₂TermFderiv n z τ) (0, 1)‖ ≤ ‖jacobiTheta₂TermFderiv n z τ‖ := by
    refine (ContinuousLinearMap.le_opNorm _ _).trans ?_
    simp_rw [Prod.norm_def, norm_one, norm_zero, max_eq_right zero_le_one, mul_one, le_refl]
  refine le_trans ?_ this
  simp_rw [jacobiTheta₂TermFderiv, jacobiTheta₂Term, FunLike.coe_smul',
...
lemma summable_jacobiTheta₂_term_fderiv_iff (z τ : ℂ) :
    Summable (jacobiTheta₂TermFderiv · z τ) ↔ 0 < im τ := by
  constructor
  ...
  · intro hτ
    ...
    refine (norm_jacobiTheta₂_term_fderiv_le n z τ).trans
      (?_ : 3 * π * |n| ^ 2 * ‖jacobiTheta₂Term n z τ‖ ≤ _)
...
lemma summable_jacobiTheta₂'_term_iff (z τ : ℂ) :
    Summable (jacobiTheta₂'Term · z τ) ↔ 0 < im τ := by
  constructor
  · rw [← summable_jacobiTheta₂_term_iff (z := z)]
    ...
    have : ∀ᶠ (n : ℤ) in cofinite, n ≠ 0 :=
      Int.cofinite_eq ▸ (mem_sup.mpr ⟨eventually_ne_atBot 0, eventually_ne_atTop 0⟩)
    filter_upwards [this] with n hn
    rw [jacobiTheta₂'Term, norm_mul, ← mul_assoc]
  ...
  · refine fun hτ ↦ ((summable_pow_mul_jacobiTheta₂_term_bound
    ...
    rw [jacobiTheta₂'Term, norm_mul, ← mul_assoc, pow_one]
...
-/
def jacobiTheta₂ (z τ : ℂ) : ℂ := ∑' n : ℤ, jacobiTheta₂Term n z τ

/-- Fréchet derivative of the two-variable Jacobi theta function. -/
def jacobiTheta₂Fderiv (z τ : ℂ) : ℂ × ℂ →L[ℂ] ℂ := ∑' n : ℤ, jacobiTheta₂TermFderiv n z τ

@[deprecated (since := "2026-07-18")]
alias jacobiTheta₂_fderiv := jacobiTheta₂Fderiv

/-- The `z`-derivative of the Jacobi theta function,
`θ' z τ = ∑' (n : ℤ), 2 * π * I * n * cexp (2 * π * I * n * z + π * I * n ^ 2 * τ)`.
-/
def jacobiTheta₂' (z τ : ℂ) := ∑' n : ℤ, jacobiTheta₂'Term n z τ

lemma hasSum_jacobiTheta₂_term (z : ℂ) {τ : ℂ} (hτ : 0 < im τ) :
    HasSum (fun n ↦ jacobiTheta₂Term n z τ) (jacobiTheta₂ z τ) :=
  ((summable_jacobiTheta₂_term_iff z τ).mpr hτ).hasSum

lemma hasSum_jacobiTheta₂_term_fderiv (z : ℂ) {τ : ℂ} (hτ : 0 < im τ) :
    HasSum (fun n ↦ jacobiTheta₂TermFderiv n z τ) (jacobiTheta₂Fderiv z τ) :=
...
lemma jacobiTheta₂_fderiv_undef (z : ℂ) {τ : ℂ} (hτ : im τ ≤ 0) : jacobiTheta₂Fderiv z τ = 0 := by
  apply tsum_eq_zero_of_not_summable
...
lemma hasFDerivAt_jacobiTheta₂ (z : ℂ) {τ : ℂ} (hτ : 0 < im τ) :
    HasFDerivAt (fun p : ℂ × ℂ ↦ jacobiTheta₂ p.1 p.2) (jacobiTheta₂Fderiv z τ) (z, τ) := by
  ...
  let f : ℤ → ℂ × ℂ → ℂ := fun n p ↦ jacobiTheta₂Term n p.1 p.2
  let f' : ℤ → ℂ × ℂ → ℂ × ℂ →L[ℂ] ℂ := fun n p ↦ jacobiTheta₂TermFderiv n p.1 p.2
  ...
  simpa only [jacobiTheta₂, jacobiTheta₂Fderiv, f, f'] using!
...
lemma hasDerivAt_jacobiTheta₂_fst (z : ℂ) {τ : ℂ} (hτ : 0 < im τ) :
  ...
  { toFun := fun f ↦ f (1, 0)
    ...
    map_smul' := by simp }
  have step1 : HasSum (fun n ↦ (jacobiTheta₂TermFderiv n z τ) (1, 0))
      ((jacobiTheta₂Fderiv z τ) (1, 0)) := by
    apply eval_fst_CLM.hasSum (hasSum_jacobiTheta₂_term_fderiv z hτ)
  have step2 (n : ℤ) : (jacobiTheta₂TermFderiv n z τ) (1, 0) = jacobiTheta₂'Term n z τ := by
    simp only [jacobiTheta₂TermFderiv, smul_add, add_apply,
      FunLike.coe_smul', ContinuousLinearMap.coe_fst', Pi.smul_apply, smul_eq_mul,
      mul_one, ContinuousLinearMap.coe_snd', mul_zero, add_zero, jacobiTheta₂'Term,
      jacobiTheta₂Term, mul_comm _ (cexp _)]
  rw [funext step2] at step1
  have step3 : HasDerivAt (fun x ↦ jacobiTheta₂ x τ) ((jacobiTheta₂Fderiv z τ) (1, 0)) z :=
...
lemma continuousAt_jacobiTheta₂' (z : ℂ) {τ : ℂ} (hτ : 0 < im τ) :
  ...
  · apply Continuous.continuousOn
    unfold jacobiTheta₂'Term jacobiTheta₂Term
    fun_prop
...
lemma jacobiTheta₂_add_right (z τ : ℂ) : jacobiTheta₂ z (τ + 2) = jacobiTheta₂ z τ := by
  ...
  simp_rw [jacobiTheta₂Term, Complex.exp_add]
...
lemma jacobiTheta₂_add_left (z τ : ℂ) : jacobiTheta₂ (z + 1) τ = jacobiTheta₂ z τ := by
  refine tsum_congr (fun n ↦ ?_)
  simp_rw [jacobiTheta₂Term, mul_add, Complex.exp_add, mul_one, mul_comm _ (n : ℂ),
...
lemma jacobiTheta₂_add_left' (z τ : ℂ) :
  ...
  refine tsum_congr (fun n ↦ ?_)
  simp_rw [jacobiTheta₂Term, ← Complex.exp_add, Equiv.coe_addRight, Int.cast_add]
  ring_nf
...
lemma jacobiTheta₂_neg_left (z τ : ℂ) : jacobiTheta₂ (-z) τ = jacobiTheta₂ z τ := by
  ...
  refine tsum_congr (fun n ↦ ?_)
  simp_rw [jacobiTheta₂Term, Equiv.neg_apply, Int.cast_neg, neg_sq, mul_assoc, neg_mul_neg]
...
lemma jacobiTheta₂_conj (z τ : ℂ) :
  ...
  congr 2 with n
  simp only [jacobiTheta₂Term, mul_neg, ← exp_conj, map_add, map_neg, map_mul, map_ofNat,
    conj_ofReal, conj_I, map_intCast, neg_mul, neg_neg, map_pow]

lemma jacobiTheta₂'_add_right (z τ : ℂ) : jacobiTheta₂' z (τ + 2) = jacobiTheta₂' z τ := by
  refine tsum_congr (fun n ↦ ?_)
  simp_rw [jacobiTheta₂'Term, jacobiTheta₂Term, Complex.exp_add]
  suffices cexp (π * I * n ^ 2 * 2 : ℂ) = 1 by rw [mul_add, Complex.exp_add, this, mul_one]
  rw [(by push_cast; ring : (π * I * n ^ 2 * 2 : ℂ) = (n ^ 2 :) * (2 * π * I)), exp_int_mul,
    exp_two_pi_mul_I, one_zpow]

lemma jacobiTheta₂'_add_left (z τ : ℂ) : jacobiTheta₂' (z + 1) τ = jacobiTheta₂' z τ := by
  unfold jacobiTheta₂' jacobiTheta₂'Term jacobiTheta₂Term
  refine tsum_congr (fun n ↦ ?_)
...
lemma jacobiTheta₂'_add_left' (z τ : ℂ) :
  ...
  have (n : ℤ) : jacobiTheta₂'Term n (z + τ) τ =
      cexp (-π * I * (τ + 2 * z)) * (jacobiTheta₂'Term (n + 1) z τ -
      2 * π * I * jacobiTheta₂Term (n + 1) z τ) := by
    simp only [jacobiTheta₂'Term, jacobiTheta₂Term]
...
lemma jacobiTheta₂'_conj (z τ : ℂ) :
  ...
  congr 1 with n
  simp_rw [jacobiTheta₂'Term, jacobiTheta₂Term, map_mul, ← Complex.exp_conj, map_add, map_mul,
...
theorem jacobiTheta₂_functional_equation (z τ : ℂ) : jacobiTheta₂ z τ =
  ...
  unfold jacobiTheta₂ jacobiTheta₂Term
  have h2 : 0 < (-I * τ).re := by
