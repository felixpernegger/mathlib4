...
* `DividedPowers.subDPIdealInfOfQuot` : if there is a dp-structure on `I⬝(A/J)` such that the
...
def subDPIdealInfOfQuot {A : Type*} [CommRing A] {I : Ideal A} {hI : DividedPowers I}
...
alias subDPIdeal_inf_of_quot := subDPIdealInfOfQuot
