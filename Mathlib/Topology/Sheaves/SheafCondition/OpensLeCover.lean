...
@[simps]
def generateEquivalenceOpensLeFunctor' :
...
alias generateEquivalenceOpensLe_functor' := generateEquivalenceOpensLeFunctor'
...
category of opens contained in some `U i`. -/
@[simps]
def generateEquivalenceOpensLeInverse' (hY : Y = iSup U) :
    OpensLeCover U ⥤
...
@[deprecated (since := "2026-07-18")]
alias generateEquivalenceOpensLe_inverse' := generateEquivalenceOpensLeInverse'
...
def generateEquivalenceOpensLe (hY : Y = iSup U) :
    (ObjectProperty.FullSubcategory
      fun f : Over Y => (Sieve.generate (presieveOfCoveringAux U Y)).arrows f.hom) ≌
    OpensLeCover U where
  functor := generateEquivalenceOpensLeFunctor' _
  inverse := generateEquivalenceOpensLeInverse' _ hY
