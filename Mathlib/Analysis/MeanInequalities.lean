...
theorem inner_le_Lp_mul_Lq (hpq : p.HolderConjugate q) :
  ...
  convert! ENNReal.coe_le_coe.2 (@NNReal.inner_le_Lp_mul_Lq _ s (fun i => ENNReal.toNNReal (f i))
    (fun i => ENNReal.toNNReal (g i)) _ _ hpq) <;> simp only [one_div, ofNNReal_finsetSum, coe_mul]
  · congr! with x hx
    · exact (coe_toNNReal (H'.1 x hx)).symm
    · exact (coe_toNNReal (H'.2 x hx)).symm
  · simp only [inv_nonneg, hpq.pos.le, coe_rpow_of_nonneg, ofNNReal_finsetSum, hpq.symm.pos.le]
    congr! with x hx x hx
    · exact (coe_toNNReal (H'.1 x hx)).symm
    · exact (coe_toNNReal (H'.2 x hx)).symm
