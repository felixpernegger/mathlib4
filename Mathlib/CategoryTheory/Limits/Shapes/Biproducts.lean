...
noncomputable def whiskerIsBilimitIff {f : J → C} (c : Bicone f) (g : K ≃ J) :
  ...
  · = IsLimit.ofIsoLimit hc.isLimit (Bicone.whiskerToCone c g)
    = (IsLimit.postcomposeHomEquiv (Discrete.functorComp f g).symm _) this
    exact IsLimit.ofWhiskerEquivalence (Discrete.equivalence g) this
  · = IsColimit.ofIsoColimit hc.isColimit (Bicone.whiskerToCocone c g)
    = (IsColimit.precomposeHomEquiv (Discrete.functorComp f g) _) this
